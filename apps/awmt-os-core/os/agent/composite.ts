...
export default class CompositeAgent extends AbstractAgent<AgentState> {
	...
	async start(query: string) {
		...
		console.log("STARTING COMPOSITE AGENT");
	...
	async load_apps() {
		...
		// Apps are now loaded via the install_apps WebSocket message
		console.log("Load apps called - apps are now loaded via install_apps WebSocket message");
	}
	...
	private async open_links(selected_tools: RankedOption[]): Promise<RankedOption[]> {
		...
		for (const selected_tool of selected_links) {
			...
			console.log("SELECTED LINK", JSON.stringify(selected_link, null, 2));
