...
import { RecordedEvent } from './reason-search/view-types';
import { HoverCard, HoverCardContent, HoverCardTrigger } from './ui/hover-card';
import { ExecutionPlanData, OsDocument } from '@joshu/os-types';
...
const ExecutionPlanComponent = ({ execution_plan, doc }: { execution_plan: ExecutionPlanData, doc: OsDocument }) => {
	...
	// 	if (!ex || !last_execution_plan_update.data.tasks) {
	...
	const completedTasksCount = execution_plan?.completed_tasks?.length;
	...
	return (
		<div className="space-y-4">
						...
						{showCompletedTasks && (
							...
							>
								<div className="space-y-2 mt-2 pl-2 border-l-2 border-green-200 dark:border-green-800">
									{execution_plan?.completed_tasks?.map((task) => (
			...
			{/* Active Tasks */}
			<div className="space-y-4">
				{execution_plan?.active_tasks?.map((task) => (
					<Card key={task.id} className="w-full shadow-none hover:shadow-none">
						<div
								...
								<div>
									...
									{task.progress_percentage > 0 && (
										<div className="flex items-center gap-2 mt-1.5">
											<Progress value={task.progress_percentage} className={cn('h-1 w-24', task.status !== 'completed' && 'animate-pulse')} />
																...
																{!!subtask.progress_percentage && subtask.progress_percentage > 0 && (
																	<div className="flex items-center gap-2 mt-1.5">
																		<Progress
																			...
																			className={cn('h-1 w-24', subtask.status !== 'completed' && 'animate-pulse')}
																		/>
			...
			{/* Show thought updates if any */}
			{/* {doc.logs.length > 1 && (
				<div className="mt-4">
					<LastThought thought={doc.thoughts[doc.thoughts.length - 1].content || 'Thinking...'} />
				</div>
			)} */}
		</div>
	);
